...
// ... (前半の import や VIDEO_CONFIG, fillSingleLineTextAutoFit はそのまま)
...
export async function generateStampVideo(params, onProgress) {
    ...
    // 1. 最後のエンコード命令がGPUに届くまでわずかに待機
    await new Promise(r => setTimeout(r, 500)); 

    // 2. エンコーダーが生きているか確認してから flush
    if (encoder.state === "configured") {
        try {
            await encoder.flush();
            // 3. flushが終わってから閉じる（ここが早すぎるとエラーになる）
            encoder.close(); 
        } catch (e) {
            console.warn("Final flush failed:", e);
        }
    }

    // 4. Muxerを完了させてBlobを返す
    muxer.finalize();
    return new Blob([muxer.target.buffer], { type: 'video/mp4' });
}

// ... (後半の補助関数 getRenderedFrames, drawUI 等はそのまま)
